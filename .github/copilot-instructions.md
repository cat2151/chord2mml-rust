...
**実装済み（Phase 1 + Wave A）**: maj / min / maj7 / min7 / 7 / dim / aug / sus2 / sus4 / 6 / 9 / 11 / 13 / 7sus2 / 7sus4 / クォータル(4.N) / maj9系(maj7,add9)、全角半角の #♭、分数コード（chord over bass note）、コード進行（空白・` - `・`→`・`・`・スペースなしハイフン区切り）
**未実装（Wave B〜G で移植予定）**: add/omit/♭5/#5、`-`表記マイナー、オンコード・ポリコード・転回形、drop系ボイシング、小節・音長、度数記法、キー/スケール、インラインMML、MIDI PC、方言プリプロセス — 詳細は README のロードマップ参照
