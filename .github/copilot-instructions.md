...
**実装済み（Phase 1 + Wave A/B）**: maj / min / maj7 / min7 / 7 / dim / aug / sus2 / sus4 / 6 / 9 / 11 / 13 / 7sus2 / 7sus4 / クォータル(4.N) / maj9系(maj7,add9)、`-`/`-7`表記マイナー、修飾（add/omit/(b5)/(#5)等）、全角半角の #♭、分数コード（chord over bass note）、コード進行（空白・` - `・`→`・`・`・スペースなしハイフン区切り）
**未実装（Wave C〜G で移植予定）**: オンコード・ポリコード・転回形、drop系ボイシング、小節・音長、度数記法、キー/スケール、インラインMML、MIDI PC、方言プリプロセス — 詳細は README のロードマップ参照
...
- **コード進行の区切り**: JS版仕様の空白・` - `（スペース付きハイフン）・`→`・`・` に加え、**スペースなしハイフン（`C-F-G-C`）も第一級として恒久サポート**（2026-07-08 ユーザー決定）。`-7`（マイナー7）とは字句の最長一致で両立させる（`-`の後が音名なら区切り、`7`ならマイナー7）
